...
| **G25/G26** single plane       | Verifies via `suites/stella/unit` + `suites/polymorpha/e2e/stella-knowledge.spec.ts`                                                                                                                                                                                                      |
| **G27** cross-repo propagation | **This repo is canonical for G27 verification** — every feature walked `business-logic → stella → polymorpha → tests` must prove `node scripts/sync.mjs --check` 4 SHAs `G21` + `suites/{polymorpha,business-logic,stella}` `T6` `concurrency>1` `G18`; single-repo insertion fails `G22` | Business-logic `G27` mirror — `suites/business-logic/python` via `UPSTREAMS` `git ls-remote`                                                                                                                                | Stella `G27` mirror — `suites/stella/unit` via `UPSTREAMS` `Polymorpha-io/polymorpha-stella`                                                                                                          | Polymorpha `G27` canonical — `polymorpha` declares `Affected Repos` + central suite owner                                                                                                                                                                                           | `T6` `BrainService → KnowledgeService.search()` only `KnowledgeRecord` boundary | N/A | **Authoritative** `G25`/`G26` — all artifacts `→ KnowledgeRecord.kind → EmbeddingService → VectorStore` behind `KnowledgeService.search()` | `BrainService → KnowledgeService.search()` only; no `BrainService → UserLibrary/DatasetRepresentationService/VectorStore` |

| **G28** | **Ecosystem Sync — always pull & push all 4 repos (never single-repo stale).** Before ANY work: `git -C <each>/ pull --ff-only origin main` for `polymorpha`, `polymorpha-business-logic`, `polymorpha-stella`, `polymorpha-tests` — verify `git ls-remote HEAD` SHAs match local. After ANY change: push every affected repo + `npm update @polymorpha/{business-logic,stella}` + `node scripts/sync.mjs --check` 4 SHAs `G21` before `npm run build`/`test`. Before done: `git status --short` must be `0` in all 4 checkouts. | Same `G28` — pull all 4 before work; after `business-logic` `git push origin main` `G16`, run `npm update` in consumers + `sync --check`. | Same `G28` — pull all 4; after `stella` `git push origin main` `G16b`, sync consumers. | **Canonical G28:** This repo verifies `node scripts/sync.mjs --check` 4 SHAs `G21`; `git status --short` must be `0` in `polymorpha`, `business-logic`, `stella`, `tests` before reporting done; never leave stale `polymorpha-tests` `59568f→9fd737f` lockfile drift. |
