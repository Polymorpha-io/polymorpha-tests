...
test.describe("Polymorpha E2E — Notebook Stella Aware", () => {
  test("notebook per workspace shows cells after upload and clean (guest)", async ({
    page,
  }) => {
    ...
    await expect(page.getByText(/Upload missing\.csv/i).first()).toBeVisible({
      timeout: 10_000,
    });
  });

  test("Explain button sets activeCell and Stella shows notebook_context", async ({
    page,
  }) => {
    await uploadCsv(page, "missing");
    ...
    await expect(page.locator(".stella-msg--assistant").first()).toBeVisible({
      timeout: 20_000,
    });
    ...
    const hasNotebookCell = await page.evaluate(async () => {
      try {
        ...
        return all.some(
          (r) => r.kind === "notebook_cell" || r.kind === "notebook_output",
        );
      } catch {
        return false;
      }
  ...
  test("superseded cells kept when branching (G18 audit history)", async ({
    page,
  }) => {
    await uploadCsv(page, "missing");
    await expect(page.locator(".notebook-view")).toBeVisible({
      timeout: 15_000,
    });
    await expect(page.locator(".notebook-cell").first()).toBeVisible();
  ...
  test("export .ipynb via nbformat adapter produces valid JSON", async ({
    page,
  }) => {
    await uploadCsv(page, "missing");
    await expect(page.locator(".notebook-view")).toBeVisible({
    ...
    const ipynbValid = await page.evaluate(async () => {
      try {
        ...
        return (
          ...
          typeof ipynb.metadata?.polymorpha?.workspaceId === "string"
        );
      } catch {
        return false;
      }
  ...
  test("Stella knowledge plane is unified (notebook_cell + dataset_profile) and guest isolated", async ({
    page,
  }) => {
    await uploadCsv(page, "missing");
    ...
    const result = await page.evaluate(async () => {
      ...
      const hasNotebook =
        kinds.includes("notebook_cell") || kinds.includes("notebook_output");
      const hasDataset =
        kinds.includes("dataset_profile") || kinds.includes("column_semantic");
