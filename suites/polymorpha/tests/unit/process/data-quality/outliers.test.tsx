...
describe('Outliers Data Quality', () => {
  beforeEach(() => {
    ...
    useDataStore.setState({
      ...
      cleaningConfig: buildDefaultConfig({
        ...
        rows: [{ Age: 25, Name: 'John' }, { Age: 120, Name: 'Jane' }, { Age: 30, Name: null }]
      }),
      setCleaningConfig: (cfg: any) => useDataStore.setState({ cleaningConfig: cfg }),
    } as any);
  });

  it('renders outliers tab', async () => {
    ...
    expect(screen.getByRole('heading', { name: /Outliers/i, level: 3 })).toBeInTheDocument();
  });
});
