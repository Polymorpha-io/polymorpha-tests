/**
 * G22 Central contract — source of truth in polymorpha-tests
 * Mirrored in polymorpha/tests/unit/knowledge/dataset-column.test.ts during migration (dual-resident per G22, now removed in polymorpha).
...
beforeEach(async () => {
  ...
  // Clear provider's underlying store via knowledgeStore
});
...
describe("DatasetKnowledgeProvider — column_semantic one vector per column (G22)", () => {
  it("header-only before profile: query 'What is the Age column?' returns column_semantic provenance.columns ['Age']", async () => {
    ...
    // Override types for Status categorical
    ...
    const res = await knowledgeService.search("What is the Age column?", {
      workspaceId: ws,
      limit: 5,
    });
    const hit = res.find(
      (r) =>
        ...
        r.record.provenance.columns?.includes("Age"),
    );
    ...
    // sampleCoverage must be undefined for schema
    expect(
      (hit!.record.metadata as unknown as { sampleCoverage?: unknown })
        .sampleCoverage,
    ).toBeUndefined();
  });
  ...
  it("30-column coverage: query 'What columns are available?' all 30 indexable, no +18 sentinel", async () => {
    ...
    // No sentinel record
    expect(
      recs.some((r) => r.text.includes("+") && r.text.includes("more columns")),
    ).toBe(false);
    await knowledgeStore.putMany(recs);
    const res = await knowledgeService.search("What columns are available?", {
      workspaceId: ws,
      limit: 30,
    });
    const colHits = res.filter((r) => r.record.kind === "column_semantic");
    expect(colHits.length).toBeGreaterThanOrEqual(10); // at least many, brute but all exist
    // Ensure col_30 exists
    expect(colRecs.some((r) => r.provenance.columns?.includes("col_30"))).toBe(
      true,
    );
  });
  ...
  it("profile upgrade: header-only superseded by profile-rich, no duplicate", async () => {
    ...
    // Second provide: profile-rich should overwrite same id
    ...
    const res = await knowledgeService.search("Age mean", {
      workspaceId: ws,
      limit: 5,
    });
    expect(res.some((r) => r.record.id === "dataset:up-upgrade:col:Age")).toBe(
      true,
    );
  });
  ...
  it("synthetic dataset_profile: template-only, marked synthetic_description, not empirical", async () => {
    ...
    expect(synth!.text).toContain(
      'Dataset "clean_dataset.csv" purpose: sales churn',
    );
    ...
    // No sampleCoverage for synthetic
    expect(
      (synth!.metadata as unknown as { sampleCoverage?: unknown })
        .sampleCoverage,
    ).toBeUndefined();
  });
});
